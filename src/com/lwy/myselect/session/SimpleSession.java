...
import static com.lwy.myselect.reflection.Reflection.reflectToGetProperty;
...
//		batchSqlList = null;
	...
	public void addBatch(String sql){
		singleBatchSql = configuration.getEntity(clazz.getName()).getSQLMapper(sql).getSql();
		batchObjectList = new ArrayList<>();
	}

	/**
	 * execute batch sql
	 * @param object   object or list/set
     */
	public void addBatch(Object object){
		if(object instanceof List || object instanceof Set)
			addBatch(object);
		else
			batchObjectList.add(object);
	}
	...
	public void addBatch(List<Object> objects){
		batchObjectList.addAll(objects);
	}

	/**
	 * 添加完毕，执行批处理
	 *
	 * @param count   多少数据处理一次
	 */
	@SuppressWarnings("Convert2streamapi")
	public int executeBatch(int count) throws SQLException {
		List<List<Object>> batchProperties = new ArrayList<>();
		EntityMapper entityMapper = configuration.getEntity(clazz.getName());
		for (Object o:batchObjectList) {
			batchProperties.add(Reflection.reflectToGetProperty(singleBatchSql,o,clazz,entityMapper));
		}
		return executor.executeBatch(singleBatchSql,batchProperties,count);
	}
	...
	private int executeUpdate(String sql,Object object) throws SQLException {
		...
		List<Object> propertyList = reflectToGetProperty(sqlStatement, object, clazz,entityMapper);
	...
	private Object executeQuery(String sql,Object object) throws SQLException {
		...
		List<Object> propertyList = reflectToGetProperty(sqlStatement, object, clazz,entityMapper);//属性值，插入sql的问号中
